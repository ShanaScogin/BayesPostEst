...
#'@param ci desired level for credible intervals; defaults to 0.05 and 0.95
                    ...
                    Pr = FALSE) 
{
  ...
  mcmctab <- apply(dat_wide, 1, 
                   function(x) c(Median = round(median(x), digits = 3), # Posterior median
                                 ...
                                 Lower = as.numeric(round(quantile(x, probs = c(ci[1] / 2)), digits = 3)), # Lower CI of posterior
                                 Upper = as.numeric(round(quantile(x, probs = c((1 + ci[2]) / 2)), digits = 3)), # Upper CI of posterior
  ...
  if(Pr == FALSE){
    mcmctab <- apply(dat_wide, 1, 
                     function(x) c(Median = round(median(x), digits = 3), # Posterior median
                                   SD = round(sd(x), digits = 3), # Posterior SD
                                   Lower = as.numeric(round(quantile(x, probs = c(ci[1] / 2)), digits = 3)), # Lower CI of posterior
                                   Upper = as.numeric(round(quantile(x, probs = c((1 + ci[2]) / 2)), digits = 3))))
  }
