...
#'@param modelname name of model for plotting
...
mcmcRocPrc <- function(sims, # this needs to be generalized
  ...
  area_under_roc <- data.frame(auc = auc_roc(obs = pred_obs$y_obs, pred = pred_obs$y_pred), Model = modelname)
  
  area_under_prc <- data.frame(auc = auc_pr(obs = pred_obs$y_obs, pred = pred_obs$y_pred), Model = modelname)
