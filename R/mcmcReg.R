#' This funciton creates LaTeX or HTML regression tables for MCMC Output using 
#' the `texreg` function from the `texreg` R package
#'
...
#' @param custom_coef_names an optional vector or list of vectors containing parameter
...
#' the standard \code{custom_coef_names} argument in \code{texreg} because there is no
...
#' @param custom_gof_names an optional vector or list of vectors containing
...
mcmcReg <- function(mod, 
                    ...
                    ci = 0.95, 
                    hpdi = F,
                    custom_coef_names = NULL, 
                    gof = numeric(0),
                    custom_gof_names = character(0),
                    format = 'latex', file, ...) {
  ...
  if (class(custom_coef_names) != 'list' & !is.null(custom_coef_names)) custom_coef_names <- list(custom_coef_names)
  ...
  if (class(custom_gof_names) != 'list') custom_gof_names <- list(custom_gof_names)
  ...
  if (!is.null(custom_coef_names) & !is.list(custom_coef_names)) coef_names <- list(custom_coef_names)
  if (!is.null(custom_coef_names)) coef_names <- custom_coef_names
  ...
  tr_list <- mapply(function(v, w, x, y, z) texreg::createTexreg(coef.names = v,
                                                           ...
                                                           gof = y,
                                                           gof.names = z),
                    coef_names, samps_pe, samps_ci, gof, custom_gof_names)
